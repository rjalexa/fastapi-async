...
class OpenRouterStateManager:
    ...
    async def get_state(self, force_refresh: bool = False) -> Optional[OpenRouterStateData]:
        """
        ...
        try:
            ...
            return OpenRouterStateData(
                ...
                balance=float(state_data["balance"]) if state_data.get("balance") else None,
                usage_today=float(state_data["usage_today"]) if state_data.get("usage_today") else None,
                usage_month=float(state_data["usage_month"]) if state_data.get("usage_month") else None,
                ...
                circuit_breaker_open=state_data.get("circuit_breaker_open", "false").lower() == "true",
    ...
    async def update_state(
        ...
        try:
            ...
            try:
                ...
                elif state in [OpenRouterState.ERROR, OpenRouterState.SERVICE_UNAVAILABLE, 
                              OpenRouterState.RATE_LIMITED]:
                    consecutive_failures += 1
                
                # Determine if circuit breaker should be open
                circuit_breaker_open = consecutive_failures >= 5  # Open after 5 consecutive failures
                ...
                await self.redis.expire(self.STATE_KEY, self.DEFAULT_TTL * 2)  # 10 minutes TTL
    ...
    async def should_skip_api_call(self) -> tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # Check rate limiting
            if (state.state == OpenRouterState.RATE_LIMITED and 
                state.rate_limit_reset and 
                datetime.now(timezone.utc) < state.rate_limit_reset):
