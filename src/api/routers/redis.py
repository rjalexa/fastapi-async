...
async def get_redis_pool_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        if (pool_stats.get("max_connections") and 
            pool_stats.get("in_use_connections") is not None):
            utilization = (pool_stats["in_use_connections"] / pool_stats["max_connections"]) * 100
...
def _get_pool_recommendations(pool_stats: Dict[str, Any]) -> Dict[str, str]:
    ...
    if not recommendations:
        recommendations["status"] = "Connection pool is operating within normal parameters."
