...
class WorkerOpenRouterReporter:
    ...
    async def report_api_error(
        ...
        try:
            ...
            if not lock_acquired:
                ...
                await self._log_worker_error(redis_client, worker_id, error_type, error_message, status_code)
                return False
            ...
            try:
                ...
                if current_state_data:
                    consecutive_failures = int(current_state_data.get("consecutive_failures", 0))
                
                # Increment failure count for error states
                if error_type in ["api_key_invalid", "credits_exhausted", "rate_limited", "service_unavailable", "timeout", "network_error"]:
    ...
    def _classify_error(self, error_message: str, status_code: Optional[int]) -> str:
        ...
        if any(pattern in error_lower for pattern in ["invalid api key", "unauthorized", "authentication failed"]):
            return "api_key_invalid"
        elif any(pattern in error_lower for pattern in ["insufficient credits", "credits exhausted", "quota exceeded"]):
            return "credits_exhausted"
        elif any(pattern in error_lower for pattern in ["rate limit", "too many requests"]):
            return "rate_limited"
        elif any(pattern in error_lower for pattern in ["service unavailable", "server error", "internal error"]):
            return "service_unavailable"
        elif any(pattern in error_lower for pattern in ["timeout", "timed out"]):
            return "timeout"
        elif any(pattern in error_lower for pattern in ["network", "connection", "dns"]):
            return "network_error"
        
        return "unknown_error"
    
    async def _update_metrics(self, redis_client: aioredis.Redis, state: str, is_success: bool) -> None:
