...
class WorkerRedisConfig:
    ...
    MAX_IDLE_TIME = 600  # 10 minutes before idle connections are closed (longer for workers)
...
class WorkerRedisConnectionManager:
        ...
        for attempt in range(WorkerRedisConfig.RETRY_ATTEMPTS):
            try:
                ...
                if attempt < WorkerRedisConfig.RETRY_ATTEMPTS - 1:
                    ...
                    logger.error(f"Worker Redis operation failed after {WorkerRedisConfig.RETRY_ATTEMPTS} attempts")
            except Exception as e:
                # Non-retryable error
                logger.error(f"Worker Redis operation failed with non-retryable error: {e}")
                raise
    ...
    async def get_pool_stats(self) -> dict:
        ...
        if stats["created_connections"] is not None and stats["available_connections"] is not None:
            stats["in_use_connections"] = stats["created_connections"] - stats["available_connections"]
...
class WorkerBlockingRedisClient:
    ...
    async def blpop(self, keys, timeout: int = 0):
        ...
        async with self.connection_manager.get_connection(timeout=socket_timeout) as redis_client:
