...
async def get_next_task(redis_conn: redis.Redis, retry_ratio: float = 0.3) -> Optional[str]:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=settings.openrouter_timeout) as client:
            ...
            if response.status_code == 200:
                ...
                error_class = ERROR_CLASSIFICATIONS.get(response.status_code, TransientError)
                raise error_class(f"OpenRouter API error: {response.status_code} - {response.text}")
