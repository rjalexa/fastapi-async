...
class TestRedisService:
    ...
    async def test_initialize_optimized_success(self, redis_service):
        ...
        with patch(
            "src.api.services.initialize_redis", return_value=mock_manager
    ...
    async def test_initialize_fallback_to_simple(self, redis_service):
        ...
        with patch(
            "src.api.services.initialize_redis",
            side_effect=Exception("Optimized failed"),
        ), patch(
            "src.api.services.initialize_simple_redis", return_value=mock_simple_manager
        ), patch(
            "src.api.services.get_simple_redis", return_value=mock_simple_redis
    ...
    async def test_initialize_basic_fallback(self, redis_service):
        ...
        with patch(
            ...
            "src.api.services.initialize_simple_redis",
            side_effect=Exception("Simple failed"),
...
class TestQueueService:
    ...
    async def test_calculate_adaptive_retry_ratio(self, queue_service):
        ...
        # Warning level
        with patch.object(settings, "retry_queue_warning", 100), patch.object(
            settings, "retry_queue_critical", 200
        ):
...
class TestHealthService:
    ...
    async def test_check_health_all_healthy(self, health_service):
        ...
        with patch.object(
            health_service, "_check_workers_via_redis", return_value=True
        ):
    ...
    async def test_check_health_workers_unhealthy(self, health_service):
        ...
        with patch.object(
            health_service, "_check_workers_via_redis", return_value=False
        ):
            result = await health_service.check_health()
    ...
    async def test_check_queue_activity_with_recent_completions(self, health_service):
        ...
        current_time = time.time()
        recent_completion = datetime.fromtimestamp(
            current_time - 60
    ...
    async def test_check_queue_activity_exception(self, health_service):
        ...
        health_service.redis_service.redis.scan_iter.side_effect = Exception(
            "Redis error"
        )
