...
class TestSignalHandler:
    ...
    def test_signal_handler_sigint(self, mock_logger, mock_exit):
        ...
        mock_logger.info.assert_called_with(
            "Received signal 2, shutting down consumer..."
        )
    ...
    def test_signal_handler_sigterm(self, mock_logger, mock_exit):
        ...
        mock_logger.info.assert_called_with(
            "Received signal 15, shutting down consumer..."
        )
        mock_exit.assert_called_with(0)
    ...
    def test_signal_handler_custom_signal(self, mock_logger, mock_exit):
        ...
        mock_logger.info.assert_called_with(
            "Received signal 99, shutting down consumer..."
        )
        mock_exit.assert_called_with(0)
...
class TestMainFunction:
    ...
    def test_main_signal_registration(self, mock_logger, mock_signal):
        ...
        with patch("redis.from_url", side_effect=Exception("Stop test")), patch(
            "src.worker.consumer.settings"
    ...
    def test_main_redis_connection_setup(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", return_value=1000), patch("time.sleep"), patch(
            "socket.gethostname", return_value="test-host"
        ), patch("os.getpid", return_value=12345), patch("uuid.uuid4") as mock_uuid:
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_main_worker_id_generation(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_main_heartbeat_update(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", side_effect=time_values), patch(
            "socket.gethostname", return_value="test-host"
        ), patch("os.getpid", return_value=12345), patch("uuid.uuid4") as mock_uuid:
            mock_uuid.return_value = MagicMock()
            mock_uuid.return_value.__str__ = MagicMock(return_value="test-uuid-1234")
    ...
    def test_main_task_processing(
        ...
        with patch("time.time", return_value=1000), patch(
            ...
            "uuid.uuid4"
        ) as mock_uuid, patch("src.worker.consumer.celery_app") as mock_celery_app:
            mock_uuid.return_value = MagicMock()
            mock_uuid.return_value.__str__ = MagicMock(return_value="test-uuid-1234")
        ...
        mock_celery_app.send_task.assert_called_with(
            "summarize_text", args=["test-task-123"]
        )
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_main_queue_priority_logic(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", return_value=1000), patch(
            ...
            "uuid.uuid4"
        ) as mock_uuid, patch("random.random", return_value=0.5), patch(
            "src.worker.consumer.calculate_adaptive_retry_ratio", return_value=0.3
        ):
            mock_uuid.return_value = MagicMock()
            mock_uuid.return_value.__str__ = MagicMock(return_value="test-uuid-1234")
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_main_redis_error_handling(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", return_value=1000), patch(
            "time.sleep"
        ) as mock_sleep, patch("socket.gethostname", return_value="test-host"), patch(
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_main_unexpected_error_handling(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
...
class TestConsumerIntegration:
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_full_consumer_workflow(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", return_value=1000), patch(
            ...
            "src.worker.consumer.celery_app"
        ) as mock_celery_app, patch("random.random", return_value=0.5), patch(
            "src.worker.consumer.calculate_adaptive_retry_ratio", return_value=0.3
        ):
            mock_uuid.return_value = MagicMock()
            mock_uuid.return_value.__str__ = MagicMock(return_value="test-uuid-1234")
    ...
    @patch("redis.from_url")
    @patch("src.worker.consumer.settings")
    def test_consumer_logging_integration(
        self, mock_settings, mock_redis_from_url, mock_logger, mock_signal
    ):
        ...
        with patch("time.time", return_value=1000), patch(
            ...
            "uuid.uuid4"
        ) as mock_uuid, patch("src.worker.consumer.celery_app"):
            mock_uuid.return_value = MagicMock()
            mock_uuid.return_value.__str__ = MagicMock(return_value="test-uuid-1234")
        ...
        assert any(
            "Starting AsyncTaskFlow Redis Queue Consumer" in msg for msg in log_calls
        )
