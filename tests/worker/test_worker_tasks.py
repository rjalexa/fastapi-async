...
from src.worker.tasks import (
    summarize_text_with_pybreaker,
    extract_pdf_with_pybreaker,
    PermanentError,
    TransientError,
)
...
async def test_summarize_text_success(mock_openrouter_response):
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        "src.worker.tasks.call_openrouter_api", new_callable=AsyncMock
    ) as mock_api_call, patch("src.worker.tasks.load_prompt") as mock_load_prompt:
        # Setup mocks
        ...
        with patch(
            "src.api.openrouter_state.OpenRouterStateManager"
            ...
            result = await summarize_text_with_pybreaker(
...
async def test_summarize_text_api_error():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        "src.worker.tasks.call_openrouter_api"
    ) as mock_api_call, patch("src.worker.tasks.load_prompt") as mock_load_prompt:
        # Setup mocks
        mock_redis.return_value = AsyncMock()
        ...
        with patch(
            ...
            with pytest.raises(TransientError):
                await summarize_text_with_pybreaker(
                    "This is a long text that needs to be summarized."
                )
...
async def test_pdfxtract_success():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        "src.worker.tasks.load_prompt"
    ) as mock_load_prompt, patch(
        "src.worker.tasks.base64.b64decode"
    ) as mock_b64decode, patch("src.worker.tasks.convert_from_bytes") as mock_convert:
        # Setup mocks
        mock_redis.return_value = AsyncMock()
        ...
        with patch(
            ...
            sample_pdf_content = (
                "dGVzdCBwZGYgY29udGVudA=="  # base64 encoded "test pdf content"
            )
...
async def test_summarize_text_state_manager_skip():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            mock_manager_instance = AsyncMock()
            mock_manager_instance.should_skip_api_call.return_value = (
                True,
            ...
            with pytest.raises(
                TransientError,
                match="OpenRouter service unavailable: Service temporarily unavailable",
            ):
                await summarize_text_with_pybreaker("This is a test text.")
...
async def test_pdfxtract_state_manager_skip():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            with pytest.raises(
                TransientError,
                match="OpenRouter service unavailable: Rate limit exceeded",
            ):
                await extract_pdf_with_pybreaker("dGVzdA==", "test.pdf")
...
async def test_summarize_text_prompt_loading_error():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            # Call the task function and expect a PermanentError
            with pytest.raises(
                PermanentError, match="Prompt error: Prompt file not found"
            ):
                await summarize_text_with_pybreaker("This is a test text.")
...
async def test_pdfxtract_prompt_loading_error():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            # Call the task function and expect a PermanentError
            with pytest.raises(
                PermanentError, match="PDF extraction error: Invalid prompt format"
            ):
                await extract_pdf_with_pybreaker("dGVzdA==", "test.pdf")
...
async def test_pdfxtract_poppler_dependency_error():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            with pytest.raises(
                PermanentError,
                match="PDF extraction dependency error: poppler not installed and in PATH",
            ):
                await extract_pdf_with_pybreaker("dGVzdA==", "test.pdf")
...
async def test_pdfxtract_page_processing_failure():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        "src.worker.tasks.call_openrouter_api"
    ), patch("src.worker.tasks.load_prompt") as mock_load_prompt, patch(
        "src.worker.tasks.base64.b64decode"
    ) as mock_b64decode, patch("src.worker.tasks.convert_from_bytes") as mock_convert:
        # Setup mocks
        mock_redis.return_value = AsyncMock()
...
async def test_summarize_text_circuit_breaker_error():
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            with pytest.raises(
                TransientError,
                match="OpenRouter service protection: Circuit breaker is open",
            ):
                await summarize_text_with_pybreaker("This is a test text.")
    ...
    with patch("src.worker.tasks.get_async_redis_connection") as mock_redis, patch(
        ...
        with patch(
            ...
            mock_api_call.side_effect = Exception(
                "API error status_code=401 Unauthorized"
            )
            with pytest.raises(
                PermanentError,
                match="OpenRouter API error: API error status_code=401 Unauthorized",
            ):
                await summarize_text_with_pybreaker("This is a test text.")

            # Test transient error with status code
            mock_api_call.side_effect = Exception(
                "API error status_code=429 Rate limited"
            )
