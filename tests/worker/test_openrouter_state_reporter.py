...
class TestWorkerOpenRouterReporter:
    ...
    def mock_redis(self):
        ...
        mock_pipeline.hincrby.return_value = None
    ...
    async def test_initialization_without_redis(self):
        ...
        with patch(
            "worker.openrouter_state_reporter.get_worker_standard_redis"
        ) as mock_get_redis:
...
"""
        ...
        with patch(
            "builtins.open",
            mock_open_multiple_files({"/proc/self/cgroup": mock_cgroup_content}),
        ):
    ...
    async def test_classify_error_by_status_code(self, state_reporter):
        ...
        assert (
            state_reporter._classify_error("Some error", 503) == "service_unavailable"
        )
    ...
    async def test_classify_error_by_message(self, state_reporter):
        ...
        assert (
            state_reporter._classify_error("Invalid API key", None) == "api_key_invalid"
        )
        assert (
            state_reporter._classify_error("Insufficient credits", None)
            == "credits_exhausted"
        )
        assert (
            state_reporter._classify_error("Rate limit exceeded", None)
            == "rate_limited"
        )
        assert (
            state_reporter._classify_error("Service unavailable", None)
            == "service_unavailable"
        )
    ...
    async def test_report_api_error_lock_failed(self, state_reporter, mock_redis):
        ...
        result = await state_reporter.report_api_error(
            error_message="Some error", status_code=500
        )
    ...
    async def test_log_worker_error(self, state_reporter, mock_redis):
        ...
        await state_reporter._log_worker_error(
            mock_redis, "worker-123", "rate_limited", "Rate limit exceeded", 429
        )
    ...
    async def test_convenience_functions(self, mock_redis):
        ...
        with patch(
            "worker.openrouter_state_reporter.get_openrouter_reporter"
        ) as mock_get_reporter:
            ...
            from worker.openrouter_state_reporter import (
            ...
            assert result is True
            mock_reporter.report_api_error.assert_called_once_with(
                "Test error", 500, None
            )
    ...
    async def test_redis_connection_fallback(self, state_reporter):
        ...
        with patch(
            ...
            with patch(
                "worker.openrouter_state_reporter.aioredis.from_url"
            ) as mock_from_url:
