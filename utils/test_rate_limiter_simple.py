...
class MockRedis:
    ...
    async def eval(self, script, num_keys, *args):
        ...
        if bucket['tokens'] >= tokens_requested:
            ...
            return [1, bucket['tokens'], bucket['capacity'], bucket['refill_rate']]  # Success
        else:
            tokens_needed = tokens_requested - bucket['tokens']
            wait_time = tokens_needed / bucket['refill_rate']
            return [0, bucket['tokens'], bucket['capacity'], bucket['refill_rate'], wait_time]  # Failure
    ...
    async def hmget(self, key, *fields):
        ...
        result = []
        for field in fields:
            result.append(str(bucket.get(field, 0)) if bucket.get(field) is not None else None)
        return result
...
async def main():
    ...
    try:
        ...
        print("For full testing with real Redis, ensure Redis is running and accessible.")
