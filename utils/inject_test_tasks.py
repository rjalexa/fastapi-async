...
class TaskInjector:
    ...
    async def create_task(self, content: str, task_number: int) -> Dict[str, Any]:
        ...
        task_data = {"content": f"Test task #{task_number}: {content}"}
    ...
    async def inject_tasks(
        self, count: int = 10, delay_between: float = 0.5, batch_size: int = 1
    ) -> List[Dict[str, Any]]:
        """
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if (
                            ...
                            print(
                                f"  Task {task_num} ({task_id[:8]}...): {current_state}"
                            )
        ...
        for task_info in self.created_tasks:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"  ⚠️  Task {task_info['task_number']} delete returned {response.status_code}"
                    )
            except Exception as e:
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Inject test tasks for frontend testing"
    )
    parser.add_argument(
        "--count", type=int, default=10, help="Number of tasks to create (default: 10)"
    )
    parser.add_argument(
        "--delay",
        type=float,
    ...
    parser.add_argument(
        "--cleanup", action="store_true", help="Clean up created tasks after completion"
    )
    parser.add_argument(
        "--show-queue-status",
        action="store_true",
    ...
    async with TaskInjector(args.url) as injector:
        ...
        results = await injector.inject_tasks(
            count=args.count, delay_between=args.delay
        )
