...
class MonitorSettings(BaseSettings):
    ...
    # Alert Thresholds
    credit_warning_threshold: float = Field(
        default=10.0, env="CREDIT_WARNING_THRESHOLD"
    )  # $10
    credit_critical_threshold: float = Field(
        default=5.0, env="CREDIT_CRITICAL_THRESHOLD"
    )  # $5
        ...
        async with httpx.AsyncClient() as client:
            ...
            if isinstance(data, dict):
                ...
                data_section = data.get(
                    "data", data
                )  # Use data section if exists, otherwise use root
...
async def check_alert_cooldown(
    redis_conn: aioredis.Redis, alert_type: str, cooldown_hours: int
) -> bool:
    """
...
async def send_alert(
    redis_conn: aioredis.Redis, alert_type: str, message: str, credit_info: Dict
) -> None:
    """
...
async def monitor_credits(settings: MonitorSettings) -> Dict:
    ...
    try:
        ...
        if balance <= settings.credit_critical_threshold:
            if await check_alert_cooldown(
                redis_conn, "credit_critical", settings.alert_cooldown_hours
            ):
        ...
        # Check for warning threshold (only if not already critical)
            ...
            if await check_alert_cooldown(
                redis_conn, "credit_warning", settings.alert_cooldown_hours
            ):
        ...
        monitoring_status = {
            ...
            if balance <= settings.credit_warning_threshold
            else "ok",
...
async def get_credit_history(settings: MonitorSettings, hours: int = 24) -> Dict:
    ...
    try:
        ...
        timeline_entries = await redis_conn.zrangebyscore(
            "openrouter:credits:timeline", cutoff_time, "+inf", withscores=True
        )
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "check":
            ...
            try:
                ...
                if history["entries"]:
                    ...
                    for entry in history["entries"][-10:]:  # Show last 10 entries
                        ...
                        print(
                            f"   {timestamp.strftime('%Y-%m-%d %H:%M:%S')} - Balance: ${entry['balance']:.2f}"
                        )
                else:
            ...
            print(
                f"🔄 Starting continuous credit monitoring (check every {settings.check_interval_minutes} minutes)"
            )
            ...
            try:
                while True:
                    try:
                        ...
                        print(
                            f"[{timestamp}] Balance: ${result['balance']:.2f} | Status: {result['status'].upper()}"
                        )

                        if result["alerts_sent"]:
                            print(
                                f"              Alerts sent: {', '.join(result['alerts_sent'])}"
                            )
