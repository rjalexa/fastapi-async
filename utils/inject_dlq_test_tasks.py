...
class DLQTaskInjector:
    ...
    async def create_dlq_task(self, content: str, task_number: int, error_scenario: str) -> Dict[str, Any]:
        ...
        state_history = [
            ...
            {"state": "ACTIVE", "timestamp": (now - timedelta(hours=4, minutes=45)).isoformat()},
            {"state": "FAILED", "timestamp": (now - timedelta(hours=4, minutes=30)).isoformat()},
            {"state": "PENDING", "timestamp": (now - timedelta(hours=3, minutes=45)).isoformat()},
            {"state": "ACTIVE", "timestamp": (now - timedelta(hours=3, minutes=30)).isoformat()},
            {"state": "FAILED", "timestamp": (now - timedelta(hours=3, minutes=15)).isoformat()},
            {"state": "PENDING", "timestamp": (now - timedelta(hours=2, minutes=30)).isoformat()},
            {"state": "ACTIVE", "timestamp": (now - timedelta(hours=2, minutes=15)).isoformat()},
    ...
    async def inject_dlq_tasks(self, count: int = 3) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            print(f"Creating DLQ task {task_number}/{count} with {error_scenario} scenario...")
    ...
    async def verify_dlq_tasks(self) -> Dict[str, Any]:
        ...
        for task_info in self.created_tasks:
            ...
            try:
                ...
                status = "✅" if (in_dlq_queue and has_task_data and correct_state) else "❌"
        ...
        return {
            ...
            "success_rate": (valid_count / len(self.created_tasks)) * 100 if self.created_tasks else 0
        }
    ...
    async def cleanup_dlq_tasks(self):
        ...
        for task_info in self.created_tasks:
            ...
            try:
                ...
                print(f"  ❌ Failed to clean up DLQ task {task_info['task_number']}: {e}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Inject test tasks directly into DLQ for frontend testing")
    parser.add_argument(
        "--count", 
        type=int, 
        default=3, 
