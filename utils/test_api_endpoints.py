...
class APITester:
    ...
    async def test_streaming_endpoint(
        self, method: str, path: str, expected_status: int = 200, description: str = ""
    ) -> Dict[str, Any]:
    ...
    async def test_health_endpoints(self):
        ...
        await self.test_endpoint(
            "GET", "/api/v1/workers/", description="Worker health check"
        )
        await self.test_endpoint(
            "POST",
            "/api/v1/workers/reset-circuit-breaker",
            description="Reset circuit breakers",
        )
        await self.test_endpoint(
            "POST",
            "/api/v1/workers/open-circuit-breaker",
    ...
    async def test_task_management(self, task_id: Optional[str] = None):
        ...
        if task_id:
            ...
            await self.test_endpoint(
                "GET", f"/api/v1/tasks/{task_id}", description=f"Get task {task_id}"
            )
        ...
        await self.test_endpoint(
            "POST",
            "/api/v1/tasks/requeue-orphaned",
            description="Requeue orphaned tasks",
        )
    ...
    async def test_queue_endpoints(self):
        ...
        # Test queue status
        await self.test_endpoint(
            "GET", "/api/v1/queues/status", description="Get queue status"
        )
        ...
        # Test DLQ tasks
        await self.test_endpoint(
            "GET", "/api/v1/queues/dlq", description="Get DLQ tasks"
        )
        await self.test_endpoint(
            "GET",
            "/api/v1/queues/dlq",
    ...
    def print_summary(self):
        ...
        for result in self.test_results:
            if result["success"]:
                print(
                    f"  ✅ {result['method']} {result['path']} - {result['actual_status']}"
                )
