...
def analyze_error_patterns():
    ...
    for task_id in dlq_task_ids:
        ...
        if error_history:
            ...
            for i, error_entry in enumerate(error_history, 1):
                ...
                if "HTTP" in error_msg and any(code in error_msg for code in ["429", "502", "503", "500", "401", "402", "403", "404"]):
    ...
    if circuit_breaker_triggers:
        ...
        for trigger in circuit_breaker_triggers:
            print(f"   Task {trigger['task_id']}: {trigger['error']} at {trigger['timestamp']}")
