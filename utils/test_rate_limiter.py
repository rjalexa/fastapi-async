...
async def test_rate_limiter_basic():
    ...
    await global_rate_limiter.update_rate_limit_config(5, "10s")  # 5 requests per 10 seconds
    ...
    print(f"   Config: {status['config']['requests']} requests per {status['config']['interval']}")
    ...
    for i in range(7):  # Try to get more tokens than capacity
        ...
        # Show current status
        status = await get_rate_limit_status()
        print(f"      Remaining tokens: {status['current_tokens']:.2f} ({status['utilization_percent']:.1f}% utilized)")
...
async def test_rate_limiter_with_openrouter_config():
    ...
    print(f"   Burst of {burst_size} requests: {successful} successful in {elapsed:.3f}s")
...
async def test_rate_limiter_recovery():
    ...
    await global_rate_limiter.update_rate_limit_config(10, "5s")  # 10 requests per 5 seconds
    ...
    for i in range(6):  # Monitor for 6 seconds
        ...
        print(f"   [{timestamp}] Tokens: {status['current_tokens']:.2f} ({status['utilization_percent']:.1f}% utilized)")
...
async def simulate_multiple_workers():
    ...
    # Reset and configure
    await global_rate_limiter.update_rate_limit_config(20, "10s")  # 20 requests per 10 seconds
    ...
    async def worker_simulation(worker_id: int, requests: int):
        ...
        print(f"   Worker {worker_id}: {successful} successful, {failed} failed, avg wait: {avg_wait:.3f}s")
    ...
    for worker_id in range(num_workers):
        task = asyncio.create_task(worker_simulation(worker_id + 1, requests_per_worker))
        tasks.append(task)
