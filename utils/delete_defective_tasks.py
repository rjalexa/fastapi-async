...
def find_defective_tasks(redis_url: str = "redis://localhost:6379/0") -> List[Dict[str, Any]]:
    """
        ...
        for key in r.scan_iter("task:*"):
            ...
            if is_defective_task(task_data):
                defective_info = {
                    ...
                    "content_preview": task_data.get("content", "")[:100] + "..." if task_data.get("content", "") else "missing"
                }
...
def delete_defective_tasks(redis_url: str = "redis://localhost:6379/0", dry_run: bool = False) -> int:
    """
    ...
    # Confirm deletion
    response = input(f"Are you sure you want to delete {len(defective_tasks)} defective tasks? (yes/no): ")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Delete defective tasks from Redis"
    )
    parser.add_argument(
        "--redis-url",
